...
const MapBoundsAdjuster: React.FC<{
  ...
  useEffect(() => {
    ...
    if (window.google?.maps?.LatLngBounds) {
      ...
      // Avoid excessive zoom when there is only one marker
...
export const GoogleMapView: React.FC<GoogleMapViewProps> = ({
  ...
  const [roadPath, setRoadPath] = useState<Coordinate[]>([]);
  ...
  useEffect(() => {
    if (selectedPlaceId) {
      const foundItem = displayItems.find(
        (i) =>
          i.place_id === selectedPlaceId ||
          i.place?.id === selectedPlaceId
      );

      if (foundItem) {
        const place =
          foundItem.place ||
          allPlaces.find((p) => p.id === foundItem.place_id);
  ...
  const validStops = displayItems
    .map((item, index) => {
      const place =
        item.place ||
        allPlaces.find((p) => p.id === item.place_id);
  ...
  useEffect(() => {
    ...
    async function calculateRoute() {
      // Clear route when there are fewer than 2 destinations
      ...
      const result = await getOSRMRoute(coordinates);
      ...
      if (result) {
        ...
        // Fallback to the original straight-line path
        // if OSRM is temporarily unavailable.
  ...
  return (
    ...
    >
      ...
      >
        ...
        {routeInfo && (
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-[10px] uppercase tracking-wide text-stone-400 font-bold">
                  Road Distance
                </div>
        ...
        {validStops.map(
          ({ item, place, index, coord }) => {
            const stopNumber = index + 1;

            const color =
              dayColors[item.day_number] || '#d97706';

            const isSelected =
              activePlace?.place.id === place.id;
        ...
        {activePlace && (
          <InfoWindow
            position={{
              lat: activePlace.place.latitude,
              lng: activePlace.place.longitude
            }}
            onCloseClick={() =>
              setActivePlace(null)
            }
            ...
            <div className="max-w-[260px] p-1 font-sans text-stone-900">
                ...
                <div className="relative mb-2 rounded-lg overflow-hidden h-28 w-full bg-stone-100">
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor:
                        dayColors[
                          activePlace.item.day_number
                        ] || '#d97706'
                    }}
                  >
                    Day {activePlace.item.day_number}
                    {' · '}
                    Stop #{activePlace.index + 1}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-1.5 pt-2 border-t border-stone-100 text-[11px] text-stone-600">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>
                    {activePlace.place.estimated_cost === 0
                      ? 'Free Entry'
                      : `₹${activePlace.place.estimated_cost}/person`}
                  </span>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>
                    {activePlace.place.visit_duration} mins
                  </span>
              ...
              {activePlace.item.travel_time_from_prev > 0 && (
                <div className="mt-2 pt-1.5 border-t border-dashed border-stone-200 text-[11px] text-stone-500 flex items-center justify-between">
                  ...
                  <span className="font-medium text-stone-700">
                    {activePlace.item.travel_time_from_prev} min
                  </span>
